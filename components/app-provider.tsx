...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const localBackup = localStorage.getItem("rental-manager-data");
      if (localBackup) {
        try {
          const backup = JSON.parse(localBackup) as Partial<AppData>;
          const uuid =
            /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
          const recoveryTables = [
            ["properties", backup.properties ?? [], properties.data ?? []],
            ["units", backup.units ?? [], units.data ?? []],
            ["contracts", backup.contracts ?? [], contracts.data ?? []],
            ["monthly_charges", backup.charges ?? [], charges.data ?? []],
          ] as const;
          const recoverable = recoveryTables.map(
            ([table, localRows, remoteRows]) => {
              const remoteById = new Map(
                remoteRows.map((row) => [row.id, row]),
              );
              const rows = localRows.filter((row) => {
                if (!uuid.test(row.id)) return false;
                const remote = remoteById.get(row.id) as
                ...
                return (
                  ...
                  Boolean(
                    row.updated_at &&
                    remote.updated_at &&
                    row.updated_at > remote.updated_at,
                  )
                );
              });
              return { table, rows };
          ...
          const propertyNames = (recoverable[0].rows as Property[]).map(
            (row) => row.name,
          );
          const total = recoverable.reduce(
          ...
          if (
            total > 0 &&
            confirm(
              `この端末に未同期データが${total}件残っています。\n${propertyNames.length ? `物件: ${propertyNames.join("、")}\n` : ""}Supabaseへ復元しますか？`,
            )
          ) {
            for (const { table, rows } of recoverable) {
              if (!rows.length) continue;
              const { error } = await supabase
                .from(table)
                .upsert(rows.map((row) => ({ ...row, user_id: auth.user.id })));
              if (error) throw error;
            }
            alert(`${total}件を復元しました。画面を再読み込みします。`);
            location.reload();
            return;
          }
        } catch (error) {
          console.error("Local backup recovery failed", error);
          setSaveError(
            error instanceof Error
              ? `端末内バックアップの復元に失敗しました: ${error.message}`
              : "端末内バックアップの復元に失敗しました",
          );
        }
      }
