alter table public.contracts
  add column if not exists key_money bigint not null default 0 check (key_money >= 0);
alter table public.contracts
  add column if not exists free_rent_months integer not null default 0 check (free_rent_months >= 0);
...
begin
  select coalesce(prorate_enabled, false) into settings_prorate
    from public.app_settings where user_id = auth.uid();

  for c in select * from public.contracts
    where user_id = auth.uid() and status not in ('解約', '下書き', '終了')
      and start_date <= month_end and (end_date is null or end_date >= month_start)
  loop
    billing_date := make_date(extract(year from month_start)::integer,
      extract(month from month_start)::integer,
      least(c.billing_day, extract(day from month_end)::integer));
    payment_date_for_month := make_date(extract(year from month_start)::integer,
      extract(month from month_start)::integer,
      least(c.payment_due_day, extract(day from month_end)::integer));

    select * into charge from public.monthly_charges
      where user_id = auth.uid() and billing_month = month_start and contract_id = c.id;
    ...
    if charge.id is null and target_date >= billing_date then
      ...
      if rent_start <= month_end then
        ...
        if settings_prorate then
          ...
          active_days := least(coalesce(c.end_date, month_end), month_end)
            - greatest(rent_start, month_start) + 1;
          amount := round(c.monthly_rent::numeric * greatest(active_days, 0) / days_in_month)::bigint;
        end if;
      end if;
      if date_trunc('month', c.start_date)::date = month_start then
        amount := amount + c.key_money;
      end if;
      ...
      if amount > 0 then
        insert into public.monthly_charges(
          ...
          amount, 0, '未入金',
          case when c.key_money > 0 and date_trunc('month', c.start_date)::date = month_start
            then '自動生成（礼金含む）' else '自動生成' end
    ...
    if charge.id is not null and target_date >= payment_date_for_month
      and charge.paid_amount < charge.billed_amount
      and charge.memo not like '%[手動管理]%'
    then
      update public.monthly_charges set paid_amount = billed_amount,
        payment_date = payment_date_for_month, payment_status = '入金済',
